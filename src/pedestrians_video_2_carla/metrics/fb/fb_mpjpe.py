...
import torch
from pedestrians_video_2_carla.third_party.video_pose_3d.loss import mpjpe
...
class FB_MPJPE(Metric):
    ...
    def update(self, predictions: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]):
        try:
            ...
            prediction = prediction.reshape((-1, *original_shape[-2:]))
            target = target.reshape((-1, *original_shape[-2:]))
